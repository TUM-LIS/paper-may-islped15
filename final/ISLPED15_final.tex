...
\title{Design of Fine-grained Sequential Approximate Circuits using Probability-aware Fault Emulation} %!PN
...
\begin{keywords}
  Approximate computing, low power, fault emulation, FPGA 
\end{keywords}
...
%\subsubsection*{\bf{Motivational Example}}
...
{\bf{Voltage Over-scaling}} results in an increased probability of timing violations. The effects as well as the probability of the violation are dependent on the path through the combinational logic. Hence, each combinational logic has to be modeled by a different error probability at the register input.
{\bf{Circuit Pruning}}, i.e. reducing the precision of a circuit by removing parts of it, can also be easily modeled. Assuming a switching activity of $\alpha_{01}=0.5$ we can assign an error probability $p_e=0.5$ to model the pruning of that register. Even if the switching activity does not equal $0.5$, which is usually the case, we can determine the correct $\alpha_{01}$ of that path using simulations and assign the corresponding $p_e=\alpha_{01}$ to it.
...
The result of this approximation at algorithmic level can be seen in {\bf Figure \ref{fig:metrics_qr}} and {\bf \ref{fig:metrics_sobel}}. In case of the zero-forcing algorithm, we can see that both for the \emph{Q} and the \emph{R} part of the decomposition up to 7 bits can be pruned (visible as $p_e=0.5$). This large number is clearly dependent on the input data and the type of application and might be less for other applications. However, what can also be seen is that there is a one bit dynamics in both parts, depending on the signal quality. This seems less in the first place, but on hardware level this multiplies as we will see later. Furthermore, we can also see that, for bits that cannot be pruned completely, a certain error probability can be tolerated. For the second application, the \emph{sobel} filter, we can see similar results. The output bits correspond to a 32-bit IEEE 754 floating-point number. Many bits of the mantissa can be ignored, which is again dependent on that specific application. Exponent bits can never be pruned. In case of \emph{fdiv} and \emph{fmul} the dynamics are very large, which will result in many approximations at hardware level. In case of \emph{fsqrt}, the possible approximations are independent on the target QoS, which is why results are not shown in the Figure.
...
\begin{algorithm}[b]
  ...
  \begin{algorithmic}[1]
    \Procedure{FineApprox.}{$\mathbf{c},\mathbf{c_{coarse}},\mathbf{p_{e,max}},\delta$}
...
{\bf Figure~\ref{fig:optimization_qr}} and {\bf Table \ref{tab:approx}} are showing the approximation for benchmark circuit \emph{QR}. 
\begin{figure}[htb]
  \centering
